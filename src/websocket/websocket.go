...
func NewServer(port int, path string, timeout time.Duration, heartbeat time.Duration, isWss bool, certFile, keyFile string) *server {
	s := new(server)
	conf := config{port, path, isWss, certFile, keyFile, timeout, heartbeat}
	s.config = conf
...
func (s *server) receive(fd int64, conn net.Conn) {
	...
	heartbeat := s.config.heartbeat * time.Second
	h := time.NewTicker(heartbeat)
...
func (s *server) heartbeat(fd int64) {
	if s.conns[fd] == nil {
		return
	}
